...
export default function EditContentPage() {
  ...
  // Load content types and current content on initial render
  useEffect(() => {
    const loadData = async () => {
      try {
        // Load content types
        const typesResult = await getContentTypes({ limit: 100 });
        ...
        // Load content type of the current content
        const contentType = await getContentType(contentData.contentTypeId);
        ...
        // Initialize form with current content values
        form.reset({
  ...
  const handleTitleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Only auto-generate slug if it's empty or matches the previous auto-generation
    if (!form.getValues("slug") || form.getValues("slug") === form.getValues("title")
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/(^-|-$)/g, "")) {
      const slug = title
  ...
  const handleContentTypeChange = async (contentTypeId: string) => {
    ...
    try {
      ...
      // Initialize custom fields with default values for new fields
      const currentCustomFields = form.getValues("customFields") || {};
      const updatedCustomFields = { ...currentCustomFields };

      fields.forEach((field) => {
        // Only set default value if field doesn't exist in current custom fields
        if (updatedCustomFields[field.name] === undefined && field.defaultValue !== undefined) {
          updatedCustomFields[field.name] = field.defaultValue;
        }
      });
      ...
      {contentTypes.length === 0 ? (
        <div className="bg-muted p-6 rounded-lg text-center">
          ...
          <p className="text-muted-foreground mb-4">
            You need to create a content type before you can edit content.
          </p>
          <Button
